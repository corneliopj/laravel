...
<div class="wrapper">
                    ...
                    <div class="col-md-8">
                        <div class="card card-warning">
                            <div class="card-header">
                                <h3 class="card-title">Editar Transação Recorrente: {{ $transacaoRecorrente->description }}</h3>
                            </div>
                            <form action="{{ route('financeiro.transacoes_recorrentes.update', $transacaoRecorrente->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="card-body">
                                    <div class="form-group">
                                        <label for="description">Descrição</label>
                                        <input type="text" name="description" class="form-control @error('description') is-invalid @enderror" id="description" value="{{ old('description', $transacaoRecorrente->description) }}" required>
                                        @error('description')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="number" name="value" class="form-control @error('value') is-invalid @enderror" id="value" step="0.01" min="0.01" value="{{ old('value', $transacaoRecorrente->value) }}" required>
                                        @error('value')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="type" id="type" class="form-control @error('type') is-invalid @enderror" required>
                                            ...
                                            <option value="receita" {{ old('type', $transacaoRecorrente->type) == 'receita' ? 'selected' : '' }}>Receita</option>
                                            <option value="despesa" {{ old('type', $transacaoRecorrente->type) == 'despesa' ? 'selected' : '' }}>Despesa</option>
                                        </select>
                                        @error('type')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="frequency" id="frequency" class="form-control @error('frequency') is-invalid @enderror" required>
                                            ...
                                            @foreach ($frequencias as $key => $value)
                                                <option value="{{ $key }}" {{ old('frequency', $transacaoRecorrente->frequency) == $key ? 'selected' : '' }}>{{ $value }}</option>
                                            @endforeach
                                        </select>
                                        @error('frequency')
                                            <span class="invalid-feedback" role="alert">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="date" name="start_date" class="form-control @error('start_date') is-invalid @enderror" id="start_date" value="{{ old('start_date', $transacaoRecorrente->start_date->format('Y-m-d')) }}" required>
                                        @error('start_date')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="date" name="end_date" class="form-control @error('end_date') is-invalid @enderror" id="end_date" value="{{ old('end_date', $transacaoRecorrente->end_date ? $transacaoRecorrente->end_date->format('Y-m-d') : '') }}">
                                        @error('end_date')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
    ...
    <script>
        $(document).ready(function() {
            ...
            const initialCategoryId = "{{ old('category_id', $transacaoRecorrente->category_id) }}";
