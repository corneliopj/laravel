<nav class="main-header navbar navbar-expand navbar-white navbar-light">
    ...
    <form class="form-inline ml-3" action="{{ route('aves.search') }}" method="GET" id="searchForm">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        searchInput.addEventListener('input', function () {
            ...
            debounceTimer = setTimeout(() => { // Espera um pouco antes de fazer a busca
                ...
                fetch(`{{ route('aves.searchSuggestions') }}?query=${encodeURIComponent(query)}`)
                    .then(response => {
                        if (!response.ok) {
    ...
    suggestionItem.href = `{{ route('aves.show', ':aveId') }}`.replace(':aveId', ave.id);
