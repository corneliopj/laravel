...
class ReceitaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Obtém apenas as categorias de 'receita' para o filtro
        $categorias = Categoria::where('tipo', 'receita')->orderBy('nome')->get();

        return view('financeiro.receitas.index', compact('receitas', 'categorias', 'request'));
    }
    ...
    {
        // Obtém apenas as categorias de 'receita'
        $categorias = Categoria::where('tipo', 'receita')->orderBy('nome')->get();
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'data' => 'required|date',
            'observacoes' => 'nullable|string|max:1000', // NOVO: Validação para observacoes
        ]);

        try {
            // Cria uma nova receita com os dados validados
            Receita::create($request->all());
            return redirect()->route('financeiro.receitas.index')->with('success', 'Receita cadastrada com sucesso!');
        } catch (\Exception $e) {
            \Log::error('Erro ao cadastrar receita: ' . $e->getMessage(), ['request_data' => $request->all()]);
            return redirect()->back()->withInput()->with('error', 'Erro ao cadastrar a receita: ' . $e->getMessage());
        }
    }
