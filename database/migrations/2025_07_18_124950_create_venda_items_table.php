...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('venda_items', function (Blueprint $table) {
            $table->id();
            $table->foreignId('venda_id')->constrained('vendas')->onDelete('cascade'); // Vincula ao ID da venda
            $table->string('descricao_item'); // Descrição do item (para itens genéricos ou nome da ave)
            $table->foreignId('ave_id')->nullable()->constrained('aves')->onDelete('set null'); // Opcional: ID da ave vendida
